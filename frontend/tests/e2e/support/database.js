...
Cypress.Commands.add(
    ...
    (academicTerm) => {
        cy.task('queryDatabase', {
            query: `UPDATE users_course_executions SET course_executions_id=(SELECT id from course_executions WHERE academic_term = '${academicTerm}') WHERE users_id=(SELECT id FROM users WHERE name='Demo Teacher')`,
            credentials: credentials,
        });
    }
);
...
Cypress.Commands.add(
    'updateDemoCourseAcademicTerm',
    (academicTerm) => {
        cy.task('queryDatabase', {
            query: `UPDATE course_executions SET academic_term = '${academicTerm}' WHERE id = 1`,
            credentials: credentials,
        });
    }
);
...
Cypress.Commands.add(
    'addQuestionStats',
    (answered_question_unique, average_questions_answered, num_available, id) => {
        cy.task('queryDatabase', {
            query: `UPDATE question_stats SET answered_question_unique=${answered_question_unique}, 
                          average_questions_answered=${average_questions_answered}, 
                          num_available=${num_available} WHERE id=${id}`,
            credentials: credentials,
        });
    }
);
...
Cypress.Commands.add(
    'addQuizStats',
    (unique_quizzes_solved, average_quizzes_solved, num_quizzes, id) => {
        cy.task('queryDatabase', {
            query: `UPDATE quiz_stats SET unique_quizzes_solved=${unique_quizzes_solved},
                      average_quizzes_solved=${average_quizzes_solved},
                      num_quizzes=${num_quizzes} WHERE id=${id}`,
            credentials: credentials,
        });
    }
);
...
Cypress.Commands.add(
    'addStudentStats',
    (num_at_least3quizzes, num_more75correct_questions, num_students, id) => {
        cy.task('queryDatabase', {
...
Cypress.Commands.add('deleteStats', () => {
    dbCommand(`
        ...
        DELETE FROM teacher_dashboard_question;
        DELETE FROM student_stats;
        DELETE FROM quiz_stats;
        DELETE FROM question_stats;
        ALTER SEQUENCE student_stats_id_seq RESTART WITH 1;
        UPDATE student_stats SET id=nextval('student_stats_id_seq');
        ALTER SEQUENCE quiz_stats_id_seq RESTART WITH 1;
        UPDATE quiz_stats SET id=nextval('quiz_stats_id_seq');
        ALTER SEQUENCE question_stats_id_seq RESTART WITH 1;
        UPDATE question_stats SET id=nextval('question_stats_id_seq');
    `)
})

Cypress.Commands.add('deleteCourseExecutions', (at1, at2) => {
    dbCommand(`
        DELETE FROM auth_users;
        DELETE FROM teacher_dashboard;
        DELETE FROM student_dashboard;
        DELETE FROM users_course_executions;
        DELETE FROM users;
        DELETE FROM course_executions WHERE academic_term = '${at1}';
        DELETE FROM course_executions WHERE academic_term = '${at2}';
        ALTER SEQUENCE auth_users_id_seq RESTART WITH 1;
        UPDATE auth_users SET id=nextval('auth_users_id_seq');
        ALTER SEQUENCE teacher_dashboard_id_seq RESTART WITH 1;
        UPDATE teacher_dashboard SET id=nextval('teacher_dashboard_id_seq');
        ALTER SEQUENCE student_dashboard_id_seq RESTART WITH 1;
        UPDATE student_dashboard SET id=nextval('student_dashboard_id_seq');
        ALTER SEQUENCE users_id_seq RESTART WITH 1;
        UPDATE users SET id=nextval('users_id_seq');
        ALTER SEQUENCE course_executions_id_seq RESTART WITH 1;
        UPDATE course_executions SET id=nextval('course_executions_id_seq');
    `)
})